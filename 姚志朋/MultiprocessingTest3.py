...
for link in link_list:
    url = link
with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:executor.map(Main,link_list)
# with concurrent.futures.ProcessPoolExecutor(max_workers=5) as executor:executor.map(Main,link_list, chunksize=6)
#匯出csv檔
import pandas as pd
all_list = []
...
print("max_workers=30，執行時間為 %f 秒" % (end - start))